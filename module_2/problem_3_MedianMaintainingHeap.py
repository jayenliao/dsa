...
from problem_1 import MinHeap
from problem_2 import MaxHeap
...
class MedianMaintainingHeap:
    ...
    def get_median(self):
        ...
        # your code here
        if self.hmax.size() == self.hmin.size(): # even
            median = (self.hmax.max_element() + self.hmin.min_element()) / 2
        elif self.hmax.size() == self.hmin.size() - 1: # odd
    ...
    def balance_heap_sizes(self):
        ...
        if self.hmax.size() < self.hmin.size() - 1:
            hmin_root = self.hmin.min_element()
            self.hmin.delete_min()
            self.hmax.insert(hmin_root)
        elif self.hmax.size() > self.hmin.size():
            hmax_root = self.hmax.max_element()
            self.hmax.delete_min()
            self.hmin.insert(hmax_root)
...
if __name__ == '__main__':
    ...
    print('All tests of MedianMaintainingHeap passed!')
